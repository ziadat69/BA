...
class IdealWaypointOptimization2(GenericSR):
    BIG_M = 10 ** 9
    ...
    def __adaptive_traffic_splitting(self, current_best_util_map, s, t, d, waypoint, sp_fraction_map, best_flow_map):
        ...
        max_utilization = np.max(current_best_util_map)
    
    # Determine the split based on utilization thresholds
        if max_utilization > 0.8:
        # Increase split proportionally to the level of congestion
               split_fraction = min(0.2 + (max_utilization - 0.8) * 0.5, 1.0)
        else:
                split_fraction = 0
    
    # Update the flow map with the new split
        new_flow_map = self.__update_flow_map(sp_fraction_map, best_flow_map, s, t, d, waypoint, split_fraction)
        return split_fraction, new_flow_map
    ...
    def __waypoint_multipath_adaptive(self):
             ...
             for waypoint in range(self.__n):
              ...
              split_fraction, test_flow_map = self.__adaptive_traffic_splitting(current_best_util_map, s, t, d, waypoint, sp_fraction_map, best_flow_map)
             ...
             if test_objective < current_best_objective:
                 ...
                 current_best_split = split_fraction
    ...
    def get_name(self):
        ...
        return f"idealwaypoint_optimization2"
