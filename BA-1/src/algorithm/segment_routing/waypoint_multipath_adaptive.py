...
class WaypointMultipathAdaptive(GenericSR):
    BIG_M = 10 ** 9
    ...
    def __adaptive_traffic_splitting(self, current_best_util_map, s, t, d, waypoint, sp_fraction_map, best_flow_map):
        """ Adaptive traffic splitting based on MLU """
        split_ratios = np.linspace(0.1, 1.0, 10) 
        best_split = 1.0   
        best_objective = float('inf')

        for split in split_ratios: 
            test_flow_map = self.__update_flow_map(sp_fraction_map, best_flow_map, s, t, d, waypoint, split)
    ...
    def get_name(self):
        ...
        return f"waypoint_multipath_adaptive"
